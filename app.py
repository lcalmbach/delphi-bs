...
def get_table():
    ...
    result = st.selectbox(
        "Datenbestand", list(tables.keys()), format_func=lambda x: tables[x]
    )
    return result
...
def get_metadata(table_id):
    ...
    metadata["table"]["time_aggregation"] = json.loads(
        metadata["table"]["time_aggregation"]
    )
    sql = query["column_metadata"].format(table_id)
    df = execute_query(sql, conn)
    metadata["columns"] = df
    metadata["columns"]["column_format"] = metadata["columns"].apply(
        lambda row: json.loads(row["column_format"]), axis=1
    )
    metadata["table"]["has_agg_function_column"] = (
        len(metadata["columns"].query("stat_func.notnull()")) > 0
    )
    metadata["table"]["can_melt"] = (
        len(metadata["columns"].query("chart_field_type == 'g'")) > 0
    )
    return metadata
    ...
    if ta != {}:
        ...
        for agg_field in ta["agg_fields"]:
            ...
            if agg_field["interval"] == "date_month":
                df["day"] = 15
                df[agg_field["name"]] = pd.to_datetime(
                    [
                        f"{y}-{m}-{d}"
                        for y, m, d in zip(df["Jahr"], df["Monat"], df["day"])
                    ]
                )
...
def main():
    ...
    menu_options = [stx.TabBarItemData(id=1, title="Datenbestand", description="")]
