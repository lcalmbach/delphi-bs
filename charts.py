...
def get_group_fields(column_id_list):
    ...
    df = settings["df_all_columns"][
        (settings["df_all_columns"]["col_type_id"].isin(["O", "N"]))
        & (settings["df_all_columns"]["id"].isin(settings["group_columns"]))
    ]
    ...
    if settings["has_index"]:
        ...
        if settings["has_filter"]:
            if settings["filter"]["type"] == 5:
                ...
                df = df[
                    df[settings["filter"]["field"]] == int(settings["filter"]["value"])
                ]
    return df
    ...
    def get_criteria():
        ...
        if settings["has_filter"]:
            if settings["filter"]["type"] == 7 and settings["filter"]["value"] != []:
                ...
                settings["filter"]["type"] == 6
                and settings["filter"]["value"] != "<Alle>"
            ):
...
def get_quant_fields():
    ...
    """
    df = settings["df_all_columns"][
        (settings["df_all_columns"]["col_type_id"] == "Q")
        & (settings["df_all_columns"]["id"].isin(settings["group_columns"]))
    ]
...
def get_table_metadata():
    ...
    settings["tab_options"] = df.loc[0, "tab_options"]
    settings["time_aggregation"] = json.loads(df.iloc[0]["time_aggregation"])
    settings["is_aggregated"] = settings["time_aggregation"] != {}
    if settings["has_filter"]:
        settings["filter"] = json.loads(df.iloc[0]["filter"])
        if settings["filter"]["type"] in (6, 7):
            settings["filter"]["lookup"] = get_filter_lookup(
                settings["filter"]["field"], settings["table_name"]
            )
    ...
    settings["df_all_columns"]["column_format"] = settings["df_all_columns"].apply(
        lambda row: json.loads(row["column_format"]), axis=1
    )
...
def show_filter():
    ...
    settings["category"] = st.selectbox(
        "Kategorie",
        list(all_categories.keys()),
        format_func=lambda x: all_categories[x],
    )
    tables = get_tables_dict(settings["category"])
    settings["table"] = st.selectbox(
        "Tabelle", list(tables.keys()), format_func=lambda x: tables[x]
    )
    get_table_metadata()
    fields = get_fields_dict()
    if len(fields) > 0:
        settings["group_columns"] = st.multiselect(
            "Felder",
            list(fields.keys()),
    ...
    settings["df_selected_columns"] = settings["df_all_columns"][
        settings["df_all_columns"]["id"].isin(settings["group_columns"])
    ]
    ...
    if settings["has_filter"] > 0:
        if settings["filter"]["type"] == 4:
            ...
            settings["filter"]["value"] = st.number_input(
            ...
            settings["filter"]["value"] = st.selectbox(
                settings["filter"]["label"], options=settings["filter"]["lookup"]
            )
        elif settings["filter"]["type"] == 7:
            settings["filter"]["value"] = st.multiselect(
                settings["filter"]["label"],
                options=settings["filter"]["lookup"],
                default=settings["filter"]["lookup"],
            )
            settings["filter"]["value"] = [
                i.replace("'", "''") for i in settings["filter"]["value"]
            ]
...
def get_plot_options(df_data):
    ...
    if "x_axis_tick_freq" in plot_options:
        ...
        plot_options["x"] = alt.X(
            plot_options["x"], axis=alt.Axis(values=list(range(min_x, max_x, step_x)))
        )
    else:
    ...
    if ta != {}:
        if ta["level"] == "month":
            ...
            df["Datum"] = pd.to_datetime(
                [f"{y}-{m}-{d}" for y, m, d in zip(df["Jahr"], df["Monat"], df["Tag"])]
            )
            settings["y"] = "Datum:T"
...
def get_chart(df, plot_type, plot_options):
    ...
    def plot_barchart():
        chart = (
            alt.Chart(df).mark_bar().encode(x=plot_options["x"], y=plot_options["y"])
        )
    ...
    title = plot_options["title"] if "title" in plot_options else ""
...
def show_table(df: pd.DataFrame):
    ...
    def get_format():
        ...
        formatted_columns = settings["df_selected_columns"][
            settings["df_selected_columns"]["column_format"] != None
        ]
...
def show_chart(df: pd.DataFrame):
    ...
    _df = settings["df_all_columns"]
    id_vars = _df[
        (_df["is_id_vars_field"] == 1) & (_df["id"].isin(settings["group_columns"]))
    ]
    id_vars = list(id_vars["label"])
    value_vars = _df[
        (_df["is_value_vars_field"] == 1) & (_df["id"].isin(settings["group_columns"]))
    ]
    value_vars = list(value_vars["label"])
    if settings["is_aggregated"]:
        df, id_vars = complete_aggregated_df(df, id_vars)
        categories = list(df["Kategorie"].unique())
        for kat in categories:
            plot_options["title"] = kat
            try:
                _df = df[df["Kategorie"] == kat]
                _df = _df.melt(
                    id_vars=["Datum", "Kategorie"],
                    value_vars=value_vars,
                    var_name=plot_options["melt_var_name"],
                    value_name=plot_options["melt_value_name"],
                )
            except:
                st.warning(
                    "Mit den ausgewählten Feldern kann keine Grafik erzeugt werden"
                )
                df = pd.DataFrame()
            chart = get_chart(_df, plot_type, plot_options)
            st.altair_chart(chart)
    # wohnviertel charts, todo make more generalized
    elif plot_options["groupby"] == "selected_fields":
        chart_group_fields = settings["df_all_columns"][
            (settings["df_all_columns"]["is_chart_group_by_field"] == 1)
            & (settings["df_all_columns"]["id"].isin(settings["group_columns"]))
        ]
        chart_group_fields = list(chart_group_fields["label"])
        chart_index_field = list(
            settings["df_all_columns"][
                settings["df_all_columns"]["is_chart_index_field"] == 1
            ]["label"]
        )[0]
    ...
    # used for hospital indicators, todo: use generealized method
        ...
        for par in value_vars:
            df_melted = df[["Jahr", "Spital", par]].melt(
                id_vars=["Jahr", "Spital"],
                value_vars=[par],
                var_name="Legende",
                value_name=par,
    ...
    # if there are 2 index parameters, the second index parameter is used for the color encoding
        ...
        sel_color_column = st.selectbox(
            "Wähle Kategorie der Grafiklegende", options=id_vars[1:]
        )
        try:
            # df = df[[id_vars[0], sel_color_column, value_vars[0]]]
            df = df.melt(
                id_vars=[id_vars[0], sel_color_column],
                value_vars=value_vars,
                var_name=plot_options["melt_var_name"],
                value_name=plot_options["melt_value_name"],
            )
...
def main():
    ...
    init()
    show_filter()
    data = data = [stx.TabBarItemData(id=1, title="Tabelle", description="")]
    if "Grafik" in settings["tab_options"]:
        data.append(stx.TabBarItemData(id=2, title="Grafik", description=""))
    data.append(stx.TabBarItemData(id=3, title="Metadaten", description=""))
    action = stx.tab_bar(data, default=1)
